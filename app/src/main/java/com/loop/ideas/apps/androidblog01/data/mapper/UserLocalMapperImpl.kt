 ...
 * Lima, Peru.
 */
class UserLocalMapperImpl : UserLocalMapper {

    override suspend fun mapLocalUserToDomain(localUser: UserLocal): UserDomain {
        return UserDomain(
            localUser.id,
            localUser.name,
            localUser.twitter
    ...
    override suspend fun mapLocalUsersToDomain(localUsers: List<UserLocal>): List<UserDomain> {
        return localUsers.map { mapLocalUserToDomain(it) }
    }
}